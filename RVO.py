...
def RVO_update(X, V_des, V_current, ws_model, time_horizon=6.0, use_colregs=True):
    """ compute best velocity given the desired velocity, current velocity and workspace model"""
    ROB_RAD = ws_model['robot_radius']+0.3
    ...
    for i in range(len(X)):
        ...
        for j in range(len(X)):
            if i!=j:
                ...
                # Check COLREGS rules if enabled
                if use_colregs:
                    # Calculate relative bearing to determine if other vessel is on the right
                    dx = pB[0] - pA[0]
                    dy = pB[1] - pA[1]
                    
                    # Estimate heading from velocity if available, otherwise use position difference
                    heading_A = atan2(vA[1], vA[0]) if (vA[0] != 0 or vA[1] != 0) else 0
                    
                    # Calculate bearing in vessel's frame
                    bearing = atan2(dy, dx) - heading_A
                    # Normalize to [-pi, pi]
                    while bearing > PI:
                        bearing -= 2*PI
                    while bearing < -PI:
                    ...
                    # Check if other vessel is on the right (bearing between 0 and pi/2)
                    is_on_right = 0 < bearing < PI
                    
                    # Modify RVO based on COLREGS rules
                    if is_on_right:
                        # Vessel on right has priority - use asymmetric RVO
                        # This vessel takes more responsibility for avoiding collision
                        transl_vB_vA = [pA[0]+0.2*vB[0]+0.8*vA[0] * time_horizon, 
                                        pA[1]+0.2*vB[1]+0.8*vA[1] * time_horizon]
                    else:
                        # Standard RVO
                        transl_vB_vA = [pA[0]+0.5*(vB[0]+vA[0])*time_horizon, 
                                        pA[1]+0.5*(vB[1]+vA[1])*time_horizon]
                else:
                    # Standard RVO without COLREGS
                    transl_vB_vA = [pA[0]+0.5*(vB[0]+vA[0])*time_horizon, 
                                    pA[1]+0.5*(vB[1]+vA[1])*time_horizon]
                ...
                # If vessel is on the right and COLREGS is enabled, make the VO cone wider
                # This creates a larger avoidance area for vessels with right of way
                if use_colregs and is_on_right:
                    # Widen the cone by 20%
                    wider_angle = theta_BAort * 1.2
                    theta_ort_left = theta_BA + wider_angle
                    bound_left = [cos(theta_ort_left), sin(theta_ort_left)]
                    theta_ort_right = theta_BA - wider_angle
                    bound_right = [cos(theta_ort_right), sin(theta_ort_right)]
                
                RVO_BA = [transl_vB_vA, bound_left, bound_right, dist_BA, 2*ROB_RAD]
                RVO_BA_all.append(RVO_BA)
                
        # Process static obstacles (unchanged)
...
def intersect(pA, vA, RVO_BA_all):
    ...
    # Increase angular resolution (smaller step size)
    angular_resolution = 0.1  # Changed from 0.1 to 0.05 (finer)
    
    # Increase radial resolution (more samples)
    radial_samples = 10  # Changed from 5 to 10 (more samples)
    
    for theta in numpy.arange(0, 2*PI, angular_resolution):
        for rad in numpy.arange(0.02, norm_v+0.02, norm_v/radial_samples):
