...
config_path = f"/home/akash/id5090/project/cfg_polygon.yml"
...
def arrange_agents_in_polygon(config):
    ...
    radius = 10.0  # Radius of the polygon
    center = [10.0, 10.0]  # Center of the polygon
...
def arrange_agents_in_crossing_flows(config):
    """Arrange agents in crossing flows (east-west and north-south)"""
    num_agents = len(config['agents'])
    half = num_agents // 2
    
    # First half: east-west flow
    for i in range(half):
        agent_id = f"agent{i}"
        config['agents'][agent_id]['initial_pose'][0] = 0.0  # West side
        config['agents'][agent_id]['initial_pose'][1] = 10.0 + i
        config['agents'][agent_id]['initial_goal'][0] = 30.0  # East side
        config['agents'][agent_id]['initial_goal'][1] = 10.0 + i
    
    # Second half: north-south flow
    for i in range(half, num_agents):
        agent_id = f"agent{i}"
        config['agents'][agent_id]['initial_pose'][0] = 10.0 + (i-half)
        config['agents'][agent_id]['initial_pose'][1] = 0.0  # South side
        config['agents'][agent_id]['initial_goal'][0] = 10.0 + (i-half)
        config['agents'][agent_id]['initial_goal'][1] = 30.0  # North side

# Load configuration
CONFIG = yaml.safe_load(open(config_path))

# Scale to desired number of agents (e.g., 12)
num_agents = 12  # Change this to your desired number
CONFIG = scale_agents(CONFIG, num_agents)

# Add communication range parameter to CONFIG
CONFIG['simulator']['comm_range'] = 5.0  # Agents can only see others within 5 units
...
arrange_agents_in_crossing_flows(CONFIG)
...
metrics = {
    ...
    'completion_rate': 0,
    'avg_path_length': 0,
    'communication_range': float(CONFIG['simulator'].get('comm_range', float('inf'))),
...
for step in tqdm(range(CONFIG['simulator']['steps'])):
    try:
        observation = sim.step() # shape: (num_agents, num_states) (num_states = 6)
